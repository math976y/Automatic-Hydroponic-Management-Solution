...
export const appRouter = router({
	setPump: publicProcedure.input(z.boolean()).query((opts) => {
		console.log('Pump state: ', opts.input);
		lastPumpVal = opts.input;
		ee.emit('pump', lastPumpVal);
	}),
	setPhVal: publicProcedure.input(z.number()).query((opts) => {
	...
	setECVal: publicProcedure.input(z.number()).query((opts) => {
		lastECVal = { EC: opts.input };
		ee.emit('ec', lastECVal);
	}),
	setHumiditySetPoint: publicProcedure.input(z.number()).query((opts) => {
